...
// TestModalSubmitRoleSelect verifies that a modal submit interaction containing
// a role select menu (type 6) inside an ActionsRow is correctly unmarshalled,
// including selected Values and resolved Role data.
func TestModalSubmitRoleSelect(t *testing.T) {
	...
	// Verify top-level component is an ActionsRow.
	...
	// Verify nested component is a role SelectMenu.
	...
	// Verify the GetSelectMenu helper finds the correct component.
...
// TestModalSubmitChannelSelect verifies that a channel select menu (type 8) inside
// a modal submit is correctly unmarshalled, including Values and resolved Channel data.
...
// TestModalSubmitTextInput verifies that a traditional text-input-only modal submit
// still unmarshals correctly and that GetTextInput works.
func TestModalSubmitTextInput(t *testing.T) {
	...
	// GetSelectMenu should return nil for a text-only modal.
	if sel := data.GetSelectMenu("my_input"); sel != nil {
		t.Error("GetSelectMenu should return nil for a TextInput custom_id")
	}
}

// TestModalSubmitMultipleSelectMenus verifies that when a modal contains both a role
// and a channel select in separate rows, each is accessible by its custom_id.
func TestModalSubmitMultipleSelectMenus(t *testing.T) {
	...
	// Verify both resolved collections are populated.
...
// TestModalSubmitSelectInLabel verifies that a select menu wrapped inside a Label
// component (type 18) is found correctly by GetSelectMenu.
...
// TestGetSelectMenuMissing verifies that GetSelectMenu returns nil for an unknown custom_id.
